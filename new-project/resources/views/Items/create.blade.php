            ...
            <div class="mt-5 bg-white rounded-lg shadow">
                ...
                <div class="flex flex-row-reverse p-3">
                    <div class="flex-initial pl-3">
                            <div class="px-5 pb-5">
                                <form action="{{ route('items.store') }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('POST')
                                    <div class="px-5 pb-5">
                                        ...
                                        <input placeholder="Image URL" name="image" class="text-black placeholder-gray-600 w-full px-4 py-2.5 mt-2 text-base transition duration-500 ease-in-out transform border-transparent rounded-lg bg-gray-200 focus:border-blueGray-500 focus:bg-white dark:focus:bg-gray-800 focus:outline-none focus:shadow-outline focus:ring-2 ring-offset-current ring-offset-2 ring-gray-400">
                                        @error('image')
                                        <div style="color: red; font-weight: bold; font-size: 13px">{{ $message }}</div>
                                        @enderror

                                        <input placeholder="Author" name="author" class="text-black placeholder-gray-600 w-full px-4 py-2.5 mt-2 text-base transition duration-500 ease-in-out transform border-transparent rounded-lg bg-gray-200 focus:border-blueGray-500 focus:bg-white dark:focus:bg-gray-800 focus:outline-none focus:shadow-outline focus:ring-2 ring-offset-current ring-offset-2 ring-gray-400">
                                        @error('author')
                                        <div style="color: red; font-weight: bold; font-size: 13px">{{ $message }}</div>
                                        @enderror
